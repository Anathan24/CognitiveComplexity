...
import uninsubria.cognitivcomplexity.enums.Statements;
...
public class StatementAnalizer {
	...
	public int analizeStatement(Node node, int nesting) {
		...
		String statementSimpleName = node.getClass().getSimpleName();
		Statements type = Statements.getEnumFromString(statementSimpleName);
			switch(type) {
				case IF_STMT:
				...
				case STATEMENT_NOT_FOUND:
					if(node instanceof Expression) {
						...
						logger.debug("Statement Enum Type Not Found! Statement Simple Name: {}; Node: {}", statementSimpleName, node);
					}
					break;
			}
	...
	private int analizeIfStmt(Node node, int nesting) {
		...
		return complexity + ifExpressionComplexity;
	}
